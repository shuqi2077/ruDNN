...
macro_rules! bf16_via_f32 {
    ($bf16_fn:ident, $f32_fn:ident, $dim:literal, $Options:ident) => {
        pub fn $bf16_fn(
            ...
            let x_f32 = $crate::convolution::common::convert_bf16_to_f32(&x);
            let weight_f32 = $crate::convolution::common::convert_bf16_to_f32(&weight);
            let bias_f32 = bias.map(|b| $crate::convolution::common::convert_bf16_to_f32(&b));
            let result_f32 = $f32_fn(x_f32, weight_f32, bias_f32, options);
            $crate::convolution::common::convert_f32_to_bf16(&result_f32)
        }
    };
}
...
pub(crate) fn squeeze_3d_to_1d(tensor: HostTensor) -> HostTensor {
    let shape = tensor.layout().shape();
    tensor.reshape(Shape::from(alloc::vec![shape[0], shape[1], shape[4]]))
}

pub(crate) fn squeeze_3d_to_2d(tensor: HostTensor) -> HostTensor {
    let shape = tensor.layout().shape();
...
pub(crate) fn convert_bf16_to_f32(tensor: &HostTensor) -> HostTensor {
...
pub(crate) fn convert_f32_to_bf16(tensor: &HostTensor) -> HostTensor {
