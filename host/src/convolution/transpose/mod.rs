...
use ruda_core::tensor::spatial::calculate_conv_transpose_output_size;
...
) -> (HostTensor, HostTensor, ConvTransposeOptions<3>) {
...
fn conv_transpose3d_impl<T: bytemuck::Pod + Clone + Copy + Send + Sync + ruda_core::tensor::element::Element>(
    x: HostTensor,
    weight: HostTensor,
    bias: Option<HostTensor>,
    options: &ConvTransposeOptions<3>,
