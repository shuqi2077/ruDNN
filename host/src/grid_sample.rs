...
use alloc::vec;
use alloc::vec::Vec;
use ruda_core::tensor::element::cast::ToElement;
use ruda_core::tensor::spatial::{GridSampleOptions, GridSamplePaddingMode, InterpolateMode};
...
pub fn grid_sample_2d(
    ...
    if matches!(options.mode, InterpolateMode::Bicubic) {
        return bicubic::sample(tensor, grid, options);
    }
    match options.mode {
    ...
    T: Float + Element + bytemuck::Pod,
{
    ...
    HostTensor::new(bytes, Layout::contiguous(out_shape), T::dtype())
}
