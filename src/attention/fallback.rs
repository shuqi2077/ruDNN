...
use super::fallback_ops::AttentionFallbackOps;

/// Computes softmax(QKᵗ * scale) · V using separate kernels.
/// Serves as a fallback when FlashAttention is not used.
pub fn attention_fallback<B: AttentionFallbackOps>(
    query: B::FloatTensor,
    key: B::FloatTensor,
    value: B::FloatTensor,
    mask: Option<B::BoolTensor>,
    attn_bias: Option<B::FloatTensor>,
    ...
    let scale = options
        .scale
        .unwrap_or_else(|| 1.0 / Float::sqrt(*query_shape.last().unwrap() as f64));
...
fn build_causal_mask<B: AttentionFallbackOps>(attention_scores: &B::FloatTensor) -> B::BoolTensor {
    ...
    let settings = B::device_settings(&device);
