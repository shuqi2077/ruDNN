...
use ruda_kernel::tiling::{TileSize, as_cmma_layout};
...
pub fn allocate_rowwise_acc<A: Float>(
    ...
    match matmul {
        AttentionTileMatmul::Cmma(c) => {
            let matrix = unsafe {
                ruda_kernel::dsl::cmma::Matrix::<A>::uninitialized(
                    ruda_kernel::dsl::cmma::MatrixIdent::Accumulator,
                    c.matmul.tile_size.m as usize,
                    c.matmul.tile_size.n as usize,
                    c.matmul.tile_size.k as usize,
                    ruda_kernel::dsl::cmma::MatrixLayout::Undefined,
                )
            };
            let cmma = CmmaTile::<A> {
                matrix,
...
pub fn allocate_softmax_target_lhs<L: Float>(
    ...
    match matmul {
        AttentionTileMatmul::Cmma(c) => {
            let matrix = unsafe {
                ruda_kernel::dsl::cmma::Matrix::<L>::uninitialized(
                    ruda_kernel::dsl::cmma::MatrixIdent::A,
                    c.matmul.tile_size.m as usize,
                    c.matmul.tile_size.n as usize,
                    c.matmul.tile_size.k as usize,
