...
use ruda_kernel::library::tensor::launch::ViewArg;
use ruda_kernel::library::tensor::layout::fixed_dim::FixedDimLayout;
use ruda_kernel::library::tensor::layout::fixed_dim::FixedDimLayoutLaunch;
...
pub fn pool2d_direct<E: Numeric, I: Int, N: Size, S: Pool2dDirectStrategyFamily>(
    ...
    let mut accumulator = S::Pool2d::<E, I, N>::initialize(config);
    ...
    for kh in 0..kernel_size.0 {
        ...
        for kw in 0..kernel_size.1 {
            ...
            S::Pool2d::<E, I, N>::count_position(config, &mut accumulator, ih, iw);
    ...
    S::Pool2d::<E, I, N>::store(config, (b, oh, ow, c), output, indices, accumulator);
}
